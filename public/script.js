...
// ============================================================
//  script.js  —  SlideVault Frontend
//  ✅ Security: input validation before sending to server
//  ✅ Feature:  Delete with token, token saved in localStorage
// ============================================================
...
// ── Delete token storage (localStorage) ─────────────────────
// Key: presentation _id  →  Value: deleteToken
function saveDeleteToken(id, token) {
  const tokens = JSON.parse(localStorage.getItem('sv_tokens') || '{}');
  tokens[id] = token;
  localStorage.setItem('sv_tokens', JSON.stringify(tokens));
}
...
// Check if current browser uploaded this presentation
function isOwner(id) {
  return !!getDeleteToken(id);
}
...
// ── Load presentations from MongoDB ─────────────────────────
async function loadPresentations(cat='all', search='', sort='recent') {
  const p = new URLSearchParams();
  if (cat && cat !== 'all') p.set('cat', cat);
  if (search) p.set('search', search);
...
async function renderCards() {
  ...
  if (presentations.length===0) { grid.innerHTML=''; empty.style.display='block'; return; }
  empty.style.display = 'none';
  grid.innerHTML = presentations.map(cardHTML).join('');
  renderTrending();
}
...
function setGridLoading(on) {
  if (on) document.getElementById('slides-grid').innerHTML =
    `<div style="grid-column:1/-1;text-align:center;padding:60px;color:var(--muted);font-family:'IBM Plex Mono',monospace;">Loading from database…</div>`;
}
...
function cardHTML(p) {
  ...
  // Show 🗑 delete icon only if this browser uploaded it
  const deleteBtn = isOwner(p._id)
    ? `<button class="card-delete-btn" title="Delete your presentation" onclick="event.stopPropagation();confirmDelete('${p._id}','${p.title.replace(/'/g,"\\'")}')">🗑</button>`
    : '';
...
async function openModal(id) {
  try {
    ...
    // Show delete button in modal only if this browser uploaded it
    const modalDelBtn = document.getElementById('m-delete-btn');
    if (modalDelBtn) {
      modalDelBtn.style.display = isOwner(id) ? 'flex' : 'none';
    }
...
async function downloadSlide() {
  ...
  if (p.filename) triggerDownload(`/uploads/${p.filename}`, p.filename);
  showToast(`Downloading "${p.title}" ⬇`);
  renderCards(); openModal(currentModalId);
}
...
function validateForm(title, author, cat, desc) {
  if (!title || title.trim().length < 3) {
    showToast('Title must be at least 3 characters ⚠️', true); return false;
  }
  if (!author || author.trim().length < 2) {
    showToast('Author name must be at least 2 characters ⚠️', true); return false;
  }
  if (!cat) {
    showToast('Please select a category ⚠️', true); return false;
  }
  if (!desc || desc.trim().length < 10) {
    showToast('Description must be at least 10 characters ⚠️', true); return false;
  }
  return true;
}
...
async function submitPresentation() {
  ...
  if (!validateForm(title, author, cat, desc)) return;
  ...
  try {
    const result = await apiFetch(API, {method:'POST', body:fd});

    // ── SECURITY: Save the deleteToken in localStorage
    // This token is ONLY returned once — right now at upload time
    if (result.deleteToken) {
      saveDeleteToken(result._id, result.deleteToken);
      // Show the token to user so they can also note it down
      showTokenAlert(result._id, result.title, result.deleteToken);
    } else {
      showToast('Presentation published! 🎉');
      resetForm();
      await updateStats();
      setTimeout(() => showPage('home'), 1200);
    }
    ...
    btn.disabled=false;
    btn.innerHTML='<span>↑</span> Publish Presentation';
  }
}

// ── Show delete token to user after upload ───────────────────
function showTokenAlert(id, title, token) {
  // Create a modal-style alert showing the delete token
  ...
  `;
  overlay.innerHTML = `
    <div style="background:white;border-radius:16px;padding:32px;max-width:520px;width:100%;box-shadow:0 24px 80px rgba(0,0,0,0.3);">
      <div style="font-size:2rem;margin-bottom:12px;">🎉</div>
      <div style="font-family:'Playfair Display',serif;font-size:1.3rem;font-weight:700;color:#1a1714;margin-bottom:8px;">
        Published Successfully!
      </div>
      <div style="color:#4a4540;font-size:13px;margin-bottom:20px;">
        "${title}" has been uploaded to SlideVault.
      </div>

      <div style="background:#fff8f0;border:2px solid #c84b2f;border-radius:10px;padding:16px;margin-bottom:20px;">
        <div style="font-size:12px;font-weight:700;color:#c84b2f;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px;">
          ⚠️ Save Your Delete Token — Shown Only Once!
        </div>
        <div style="font-size:11px;color:#4a4540;margin-bottom:10px;">
          If you want to delete this presentation later, you will need this token. 
          It is already saved in this browser. Copy it in case you switch devices.
        </div>
        <div style="background:#1a1714;color:#e8ff5a;font-family:'IBM Plex Mono',monospace;font-size:11px;
          padding:12px;border-radius:8px;word-break:break-all;letter-spacing:0.5px;margin-bottom:10px;">
          ${token}
        </div>
        <button onclick="navigator.clipboard.writeText('${token}');this.textContent='✅ Copied!'"
          style="background:#c84b2f;color:white;border:none;padding:8px 18px;border-radius:8px;
          cursor:pointer;font-size:12px;font-weight:600;width:100%;">
          📋 Copy Token
        </button>
      </div>

      <div style="font-size:12px;color:#9a9188;margin-bottom:20px;">
        💡 This token is saved in your browser automatically. You will see a 🗑️ delete button 
        on your presentation card and in the preview modal.
      </div>

      <button onclick="this.closest('div[style]').remove();resetForm();updateStats();setTimeout(()=>showPage('home'),300);"
        style="background:#1a1714;color:white;border:none;padding:12px;border-radius:10px;
        cursor:pointer;font-size:14px;font-weight:600;width:100%;">
        Got it — Go to My Presentation →
      </button>
    </div>
  `;
